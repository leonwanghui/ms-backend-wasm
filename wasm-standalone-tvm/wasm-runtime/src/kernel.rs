...
impl KernelExecutor {
    ...
    pub fn instantiate(&mut self, wasm_kernel_file: String) -> Result<()> {
        ...
        let module = Module::from_file(&store, &wasm_kernel_file)?;
        self.instance = Some(linker.instantiate(&module)?);

        Ok(())
    }

    pub fn set_input(&mut self, input_data: Vec<Tensor>) -> Result<()> {
        let memory = self
            .instance
            .as_ref()
            .unwrap()
    ...
    pub fn run(&mut self, op_type: i32) -> Result<()> {
        // Invoke `run` export.
        let run = self
            .instance
            .as_ref()
        ...
        let out_size = run(op_type.clone(), self.wasm_addr, self.input_size)?;
        if out_size == 0 {
